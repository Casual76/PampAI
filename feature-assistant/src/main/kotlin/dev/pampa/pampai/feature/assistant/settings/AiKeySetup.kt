...
object ProviderGuides {
  ...
  fun tagline(provider: ProviderId): String = when (provider) {
    ProviderId.GROQ -> "Gratis e velocissimo, con limiti al minuto. Il primo da provare."
    ProviderId.GEMINI -> "Gratis con limiti giornalieri; vede le immagini e legge i PDF da solo."
 ...
 * inline, Rimuovi. Portata da ClasseViva Expressive: e' la stessa scheda, con le stesse parole.
 */
@Composable
fun AiKeySetup(
  provider: ProviderId,
  state: KeyState,
  saveAndVerify: suspend (ProviderId, String?) -> VerifyResult,
    ...
    Row(verticalAlignment = Alignment.CenterVertically) {
        ...
        FluidButton(
          ...
          onClick = {
            ...
            scope.launch {
              ...
              try {
                when (val result = saveAndVerify(provider, input.takeIf { it.isNotBlank() })) {
                  is VerifyResult.Ok -> {
      ...
      if (state.present && input.isBlank()) {
        ...
        FluidButton(
          ...
          style = FluidButtonStyle.Plain,
          size = FluidButtonSize.Small,
          onClick = { onRemove(provider); outcome = null },
        )
      }
    }
    outcome?.let {
