...
/** Un catalogo montato: il registry, il router costruito su di esso, e le righe per il prompt. */
...
/**
 * Chi mette insieme i tool di Aria: quelli locali (le fasi li aggiungono qui) e, dalla Fase B, quelli
 * delle app collegate letti dai loro cataloghi. Il registry si ricostruisce quando cambia qualcosa;
 * l'engine ne prende l'ultimo a ogni domanda.
 */
@Singleton
class RegistryHolder @Inject constructor() {

  private val remote = MutableStateFlow<List<RemoteToolSet>>(emptyList())

  private val current = MutableStateFlow(build(emptyList()))
  val catalog: StateFlow<MountedCatalog> = current

  /** Le app collegate montano e smontano i loro tool da qui (Fase B). */
  fun setRemote(sets: List<RemoteToolSet>) {
    remote.value = sets
  ...
  private fun build(sets: List<RemoteToolSet>): MountedCatalog {
    ...
    return MountedCatalog(registry, router, summary)
  }
}

/** I tool di un'app collegata, gia' tradotti in `AiTool` di PampAI (Fase B). */
class RemoteToolSet(
  val packageName: String,
  val appLabel: String,
  val groups: List<AiToolGroup>,
  val tools: List<AiTool<PampaiToolContext>>,
)
